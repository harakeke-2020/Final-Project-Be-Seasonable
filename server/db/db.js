...
function getFoodDetails (month, id, db = connection) {
  return db('foods')
    ...
    .where('prices.month', month)
    .where('foods.id', id)
    .select('foods.id as id', 'foods.name', 'foods.reo_name as reoName', 'prices.month', 'prices.price', 'foods.image', 'foods.nz_grown as nzGrown', 'foods.last_month as lastMonth', 'foods.first_month as firstMonth', 'foods.details', 'foods.average_price as averagePrice', 'foods.info_link as infoLink', 'foods.chart')
    .first()
}
module.exports = {
  getInSeasonFoods,
